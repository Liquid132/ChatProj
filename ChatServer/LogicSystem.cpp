...
void LogicSystem::RegisterCallBacks() {
	std::cout << "RegisterCallBacks called" << std::endl;
...
void LogicSystem::LoginHandler(shared_ptr<CSession> session, const short &msg_id, const string &msg_data) {
	...
	// 排查部分，可以删除
	bool curr_succ = RedisMgr::GetInstance()->Get(token_key, token_value);
	std::cout << "Redis GET " << token_key << " success=" << curr_succ
		<< " value=" << token_value << std::endl;
...
{
	Json::Reader reader;
	Json::Value root;					// 反序列化会将字符串序列化为Json值，将其存放进root中
	reader.parse(msg_data, root);		// 将字符串msg_data解析到root中
	auto uid_str = root["uid"].asString();
	std::cout << "user SearchInfo uid is  " << uid_str << endl;
	// 提前准备需要返回的数据
	Json::Value  rtvalue;
	// return 时将rtvalue数据序列化为字符串
	Defer defer([this, &rtvalue, session]() {
		std::string return_str = rtvalue.toStyledString();

...
- **⚠️ 常见错误**：
  ...
  int g_value = 10;    // 定义（仅一次）
    ...
    // file2.cpp (使用)
    extern int globalVar;        // 声明（不分配内存，只是告诉编译器这个变量存在）
    ```

    如果全局变量不加 static，它默认具有外部链接属性，理论上所有包含其声明的文件都可以使用。但是：不能直接 `#include` 定义变量的文件，而应该通过 `extern` 声明来使用。

    ```cpp
    // 错误做法：直接在头文件中定义
    // global.h
    int g_value = 100;  // ❌ 定义在头文件中

    // main.cpp
    #include "global.h"  // 包含定义

    // helper.cpp
    #include "global.h"  // 又包含定义
    // ❌ 链接错误：g_value 被定义了多次！
    ```
    ...
    // global.cpp
    #include "global.h"
    int g_value = 100;   // ✅ 定义（只一次）

    // main.cpp
    #include "global.h"  // 包含声明
    int main() {
        g_value = 200;   // ✅ 可以使用
        return 0;
    }
    ...
    // helper.cpp
    #include "global.h"  // 也包含声明
    void helper() {
        g_value++;       // ✅ 也可以使用
    }
    ```

    全局变量在整个程序运行期间只有**一份实例**，它的生命周期为整个程序运行期间而非函数作用域。在任何地方修改都会使其永久改变，不会因为函数结束而恢复
...
![头文件安全性](./Cpp_Picture/头文件安全性.jpg)

```text
头文件中写变量 → 用 extern 声明（不分配内存）
源文件中写变量 → 直接定义（分配内存）
头文件直接定义 → 多个文件包含 → 链接错误 ❌
```
...
对于小根堆(数组形式存储)`7, 19, 18, 27, 53, 30, 51, 48, 48, 60`，将他们从**1**开始编号
对于节点`i`
- 父节点索引`i / 2`
- 子节点索引`2 * i`,`2 * i + 1`
...
| 操作            | unique_ptr | shared_ptr | weak_ptr | 作用                              |
| ------------- | ---------- | ---------- | -------- | ------------------------------- |
| `make_xxx()`  | ✅          | ✅          | ×        | 创建智能指针                          |
| `get()`       | ✅          | ✅          | ×        | 获取原始指针，不转移所有权                   |
...
| `reset()`     | ✅          | ✅          | ×        | 释放当前资源或接管新资源                    |
| `swap()`      | ✅          | ✅          | ×        | 交换管理的资源                         |
| `use_count()` | ×          | ✅          | ✅        | 查看引用计数（`weak_ptr` 返回对应控制块的共享计数） |
| `unique()`    | ×          | ⚠️         | ×        | 是否唯一拥有者（C++20 已移除）              |
| `expired()`   | ×          | ×          | ✅        | 判断对象是否已销毁                       |
| `lock()`      | ×          | ×          | ✅        | 获取对应的 `shared_ptr`              |
| `std::move()` | ⭐ 必须       | 可用         | 可用       | 转移所有权或资源                        |
...
- 返回原始指针。
- 放弃所有权，`p`变为`nullptr`。
- 对象仍然存在。之后需要程序员负责释放资源，否则会发生内存泄漏
...
```txt
编译期：
  main.exe（含 IAT 占位符） + MathLib.dll（含导出表）

运行时（Windows Loader）：
  ① 读 main.exe 导入表 → 知道需要 MathLib.dll
  ② 找到并加载 MathLib.dll 到内存（基址 0x10000000）
  ③ 读 MathLib.dll 导出表 → 找到 add 的偏移（0x00011020）
  ④ 计算真实地址 = 0x10011020
  ⑤ 把地址填回 main.exe 的 IAT

执行 main.exe 的 call [__imp__add] 时：
  ⑥ 从 IAT 取出 0x10011020，跳转过去执行
```
...
在上例中，const修饰成员函数本身，承诺该成员函数不会修改任何成员变量
...
派生类对象对应虚函数表存放：
```text
虚函数表（vtable for Derive）：
索引0: Derive::fun1()        // 重写了 Base::fun1()
索引1: Base::B_fun2()        // 未重写，继承自 Base
索引2: Base::B_fun3()        // 未重写，继承自 Base
索引3: Derive::D_fun2()      // 派生类新增的虚函数
索引4: Derive::D_fun3()      // 派生类新增的虚函数
```

`fun1()` 被重写属于覆盖，不是隐藏。虚函数表中 `Base::fun1()` 的位置被 `Derive::fun1()` 替换了。
- 覆盖（Override）：派生类重写基类的虚函数，替换了虚函数表中对应位置的函数指针
- 隐藏（Hide）：发生在名称查找层面，与虚函数表无关
隐藏发生在非虚函数或同名但参数不同的情况下，是编译期的名称查找规则，与虚函数表无关。
...
`Base1`,`Base2`,`Base3`中都需要存放派生类的新增虚函数`Derive::D_fun1()` 等，但是他们不会将`Derive::D_fun1()` `Derive::D_fun2()` `Derive::D_fun3()`复制到每一个虚函数表中
大多数编译器实现方式：
- 主虚函数表（第一个基类 Base1 的表）中包含所有新增虚函数
- 其他虚函数表（Base2、Base3 的表）中，新增虚函数的位置存放的是跳转代码（thunk），用于调整 this 指针

派生类新增的虚函数会被复制到所有基类的虚函数表中，但对于**非第一个基类**，通常会使用 `thunk` 来调整 `this` 指针
...
**关于析构函数**
| 成员类型 | 是否自动释放 | 是否需要手动delete |
| :-- | :-- | :-- |
| 普通局部变量如`int`,`double` | 自动释放 | 不用 |
| 标准库容器如`std::string` | 自动调用自身析构 | 不用 |
| 类成员对象 | 自动调用自身析构 | 不用 |
| 动态分配数组或对象如 `new[]`,`new` | 不自动释放 | 需要`delete[]`或`delete`手动释放 |
| 原始指针如`char*`,`int*`| 只释放指针本身8个字节，不释放内存 | `delete` |
| 外部资源如文件和锁 | 不 | 要 |
...
1. 赋值、下标、解引用等：返回已有对象的引用，而不是临时对象
    | 运算符 | 返回类型 | 解释 |
    |:-- |:-- |:-- |
    | 赋值运算符`=` | `T&` | 返回左操作数的引用，支持链式赋值`a = b = c` |
    | 符合运算符`+=`等 | `T&` | 返回左操作数的引用 |
    | 下标运算符`[]` | `T&` | 返回容器元素的引用 |
    | 解引用运算符`*` | `T&` | 返回指向对象的引用 |
    | 成员访问 `->` | `T*`或`T&` | 返回指针或引用 |
    ...
    MyVector v1, v2, v3;
    v1 = v2 = v3;  // 链式赋值，必须返回引用
    v1[0] = 100;   // 修改元素，必须返回引用
    ```
2. 算术运算符、比较运算符、逻辑运算符、位运算符通常返回值（临时对象）
    ```cpp
    class MyClass {
    private:
        int data;
...
两种划分方式分别是从操作系统视角(虚拟内存分区)和编译器视角(内存分区)。实际上，从后者来看，全局区包含 BSS + 数据段；常量区则可视作数据区的一部分。
...
`detach()` 分离线程让其独立运行，分离的线程无法再使用`join`，线程的运行控制权完全交给C++运行时库，线程退出时，其资源由系统自动回收
...
1. 第 2 步没有将服务端 socket 注册到 epoll，这样有新连接到来时，服务端没办法感知这个事件，也就无法获取到已连接的 socket，那服务端自然就没机会对 socket 调用 close 函数了
2. 第 3 步没有做，有新连接到来时没有调用 accept 获取该连接的 socket，导致当有大量的客户端主动断开了连接，而服务端没机会对这些 socket 调用 close 函数，从而导致服务端出现大量 CLOSE_WAIT 状态的连接
3. 第 4 步没有做，通过 accept 获取已连接的 socket 后，没有将其注册到 epoll，导致后续收到 FIN 报文的时候，服务端没办法感知这个事件，那服务端就没机会调用 close 函数了
4. 第 6 步没有做，当发现客户端关闭连接后，服务端没有执行 close 函数，可能是因为代码漏处理，或者是在执行 close 函数之前，代码卡在某一个逻辑，比如发生死锁等
...
```text
        ...
        (通过总线传输数据，速度较慢)
         ...
         |                |                |
     +---+---+      +-----+-----+    +-----+-----+
     | 硬盘  |      | 固态硬盘  |    |  U盘/光盘 | <--- 这些都是"外存"
     | (机械) |      | (Flash)   |    | (ROM)    |      (256GB/512GB/1TB)
     +-------+      +-----------+    +----------+
```
...
- **ICMP**互联网控制报文协议
  - 网络层错误报告、诊断
  - 告知目标不可达、超时，或者用于`ping`等工具
- **ARP**地址解析协议
  - 用于将**IP地址**解析为**MAC地址**。同一局域网内，数据链路层（以太网）的通信依赖于 MAC 地址，而不是 IP 地址
  - ARP协议可以在局域网广播查询，获取目标MAC地址
- **DNS**域名系统
  - 用于将人类易读的域名（如 www.360.com）转换为机器可读的IP地址
- **TCP**传输控制协议
  - 一种面向连接的、可靠的传输层协议，提供数据完整性和顺序性。常见的应用有 HTTP、HTTPS、FTP 等
- **UDP**用户数据报协议
  - 一致种无连接的传输层协议，速度快开销小，常用于实时性高、允许少量丢包的应用，比如DNS查询
- **IP**互联网协议
  - 整个互联网的核心协议。负责将数据包从源地址发送到目标地址，其中最重要的功能是寻址（通过 IP 地址）和路由（选择数据包路径）

*Internet网络层含有四个重要协议：IP、ICMP、ARP、UDP*
...
* `Reactor`本身为同步 I/O ，基于 I/O 多路复用（如 `epoll/select`），监听事件，应用程序在收到通知后同步执行 I/O 操作。它的阻塞或者非阻塞取决于fd属性：
  - Reactor + LT：可以用阻塞 I/O（简单）
...
非阻塞 `read` 的返回条件：
- 如果缓冲区有数据：立即返回已读取的字节数
- 如果缓冲区为空：立即返回 -1，errno = EAGAIN
...
**二者均为通用的网络高并发设计模式**
| 对比维度 | `ET`(边缘触发) | `LT`(水平触发) |
| :-- | :-- | :-- |
| 逻辑 | 只在状态变化瞬间通知一次 | 状态持续“就绪”，则持续通知 |
| 数据处理 | 必须读完，否则状态变化前*不再通知 | 不必一次读完 |
| 线程模型 | 需要配合非阻塞I/O+循环 | 可以配合阻塞I/O*，简单安全 |
| 性能开销 | 唤醒次数少、性能更高 | 可能重复唤醒，开销较高 |
| 典型代表 | `Nginx、Redis` | `epoll`默认模式、`select/poll ` |

* *如果数据没读完，连接对端又发来了新数据，那么缓冲区状态会再次从“空→非空”（变化），`ET` 会再次触发通知。所以“不再通知”严格说是“在状态没有下一次变化前不再通知”，而不是“永远不再通知”

* *如果数据量极大，而你循环读取时缓冲区又恰好小于数据量，`read` 会阻塞住当前线程，导致无法处理其他连接。生产环境（如 Nginx/Redis）即使使用 LT，通常也建议配合非阻塞 I/O，以避免单连接阻塞全局
...
3. `Redis`复制
   ...
   - 当`Redis`副本示例落后于主实例几个偏移量时，他会从主实例接受剩余的命令，然后在其数据集上重放，直到同步完成
...
![真题](./Cpp_Picture/适配器.jpg)

![真题](./Cpp_Picture/网络协议.jpg)
